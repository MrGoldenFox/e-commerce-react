...
export function Product({ title, category, price, img, description }) {
	const [openModelWindow, setOpenModelWindow] = useState(false)

	return (
		<article className='relative border-1 py-2 rounded-md border-black/10 flex flex-col gap-4'>
			<img
				src={img}
				alt={title}
				className='max-w-64 mx-auto sm:max-w-54 aspect-square'
			/>
